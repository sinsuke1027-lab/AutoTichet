...
def _make_client(user_id="u1", display_name="山田 太郎", role="member"):
    from widget.clients.backend_client import BackendClient, UserInfo
    return BackendClient("https://example.hf.space", UserInfo(user_id=user_id, display_name=display_name, role=role))
...
def test_get_users_dev_returns_user_info_list_without_auth():
    ...
    assert users[0] == UserInfo(user_id="u1", display_name="山田 太郎", role="member", email="u1@example.com")
...
def test_get_users_returns_user_info_list():
    ...
    assert len(users) == 2
    assert users[0] == UserInfo(user_id="u1", display_name="山田 太郎", role="member", email="")
    assert users[1] == UserInfo(user_id="u2", display_name="田中 花子", role="admin", email="")


def test_get_projects_returns_project_info_list():
    from widget.clients.backend_client import ProjectInfo
    raw = [
        {"id": "p1", "name": "総務業務管理"},
        {"id": "p2", "name": "人事業務管理"},
    ]
...
def test_create_task_sends_correct_payload():
    ...
    dev_header = json.loads(call_kwargs.kwargs["headers"]["X-Dev-User"])
    assert dev_header["userId"] == "u1"
