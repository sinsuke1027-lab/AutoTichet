...
- [x] **F-30 遅延リスク AI 予測**: ルールベースリスクスコア・タスク一覧バッジ（高リスク/要注意）（2026-05-28）
- [x] **F-31 タスクの自動棚卸し提案**: 14日放置検知・Dashboard 棚卸しカード・キャンセルボタン（2026-05-28）
- [x] **F-12 工数自動算出**: 蓄積データから 🤖 目標工数を自動算出・提案（2026-05-28）
- [x] **F-33 テキスト抽出 UI**: メール・会議文字起こし・チャットからタスク一括抽出・確認・起票（2026-05-28）
...
## 追加改善・バックログ候補（2026-06-02 ブレスト）

Graph API 承認待ち・Phase 3 前提の機能とは独立して着手できる改善案。
`[ ]` = 未着手 / `[x]` = 完了

### 🔧 機能追加（requirements.md 未実装分）

| F-ID | 機能名 | 概要 | 優先度 |
|------|--------|------|--------|
| F-26 | マニュアルから自動生成 | 手順書・マニュアルテキストを貼り付けると Gemini がタスク群を一括提案 | Should |
| F-27 | 過去実績参照 | タスク詳細画面で同種作業の過去実績工数を参考表示 | Should |
| F-34 | 最適アサイン提案 | ワークロード・スキルタグから最適担当者を AI レコメンド | Could |
| F-35 | マイルストーン設定 | プロジェクトの節目を設定し残日数・達成度を表示 | Could |

- [ ] **F-26**: `POST /tasks/generate-from-manual` + ExtractModal 流用で UI 追加
- [ ] **F-27**: タスク詳細「工数」タブに過去実績サマリーカードを追加（`/estimate-hours` 拡張）
- [ ] **F-34**: タスク作成/編集時に担当者フィールドの横に 🤖 レコメンドボタン
- [ ] **F-35**: `project_milestones` テーブル追加・プロジェクト詳細ページにマイルストーンバー

### ⚡ 即効性高・工数少

- [x] **CSV エクスポート**: タスク一覧フィルタ結果を CSV ダウンロード（バックエンド `/tasks/export.csv` + フロントにボタン追加）（2026-06-03）
- [x] **プロジェクトアーカイブ**: プロジェクトに `archived` フラグ追加・一覧から非表示・アーカイブ済み表示切り替え
- [x] **手動並び替え**: タスク一覧・ボードで D&D 並び替え（Fractional Indexing・PATCH /{task_id}/order）（2026-06-03）

### 🌟 中期的価値

- [x] **繰り返しタスク**: `recurrence` ルール（daily/weekly/monthly）を設定 → APScheduler が次回分を自動生成（2026-06-03）
- [x] **タスク一括操作**: 一覧でチェックボックス複数選択 → ステータス/担当者一括変更（削除は除外）（2026-06-03）
  - `PATCH /tasks/bulk` エンドポイント・`TaskBulkUpdate`/`BulkUpdateResponse` モデル・テスト 4 件
  - Ant Design `rowSelection` + 画面下部固定バー（ステータス Select・担当者 Select・適用・選択解除）
- [ ] **アプリ内通知センター**: 期限超過・アサイン変更・コメントをアプリ内通知ベルで集約（Teams 通知の代替）
- [ ] **ウォッチャー機能**: 担当者以外がタスクをウォッチして進捗追跡
- [x] **マイページ**: 自分の今週の予定工数・完了率・遅延タスクに絞ったパーソナルダッシュボード（2026-06-03）

### 🔍 UX 向上

- [ ] **横断全文検索**: タスク名＋説明＋コメントを横断検索（Cmd+K コマンドパレット形式）
- [ ] **キーボードショートカット**: `N` 新規作成・`/` 検索フォーカス等のパワーユーザー向け操作
