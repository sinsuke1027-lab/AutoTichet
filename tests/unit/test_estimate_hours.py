...
def _exec_aggregate(avg_hours: float | None, task_count: int) -> MagicMock:
    """aggregate クエリ結果（.one() で row を返す）を模倣するモック。"""
    row = MagicMock()
    row.avg_hours = avg_hours
...
def test_estimate_hours_with_matching_completed_tasks() -> None:
    ...
    mock_db = AsyncMock()
    mock_db.execute = AsyncMock(return_value=_exec_aggregate(avg_hours=3.5, task_count=2))
    client = _make_client(mock_db)
    resp = client.get("/api/v1/tasks/estimate-hours?tags=backend")
    assert resp.status_code == 200
    data = resp.json()
    assert data["avg_actual_hours"] > 0
    assert data["task_count"] >= 1


def test_estimate_hours_no_matching_tags() -> None:
    """タグ一致なしのとき task_count: 0, avg_actual_hours: null を返す。"""
    mock_db = AsyncMock()
    mock_db.execute = AsyncMock(return_value=_exec_aggregate(avg_hours=None, task_count=0))
    client = _make_client(mock_db)
...
def test_estimate_hours_multiple_tags_or_search() -> None:
    ...
    mock_db = AsyncMock()
    mock_db.execute = AsyncMock(return_value=_exec_aggregate(avg_hours=5.0, task_count=3))
    client = _make_client(mock_db)
